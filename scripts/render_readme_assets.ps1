...
$stdout = Join-Path $env:TEMP ("regulatory-comment-intelligence-hub-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("regulatory-comment-intelligence-hub-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5414"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/comment-lane"; File = "02-comment-lane-proof.png"; Size = "1600,1320" },
        @{ Url = "http://127.0.0.1:$port/obligation-map"; File = "03-obligation-map-proof.png"; Size = "1600,1220" },
